...
//******************************************************************************************************************
_watchdogspeedentry::_watchdogspeedentry(_watchdogspeed*w,_tg v):_listenknotenbasis<_watchdogspeedentry>(w){
				value=v;
				watchdogspeed=w;
};
...
void _watchdogspeed::load(_datei*d){	
				_listebasis<_watchdogspeedentry>::loeschen();
				_tg f;
...
_sortentry::_sortentry(const _zeichenkette<_tt>&n,_tg c){
                name=n;
				color.setzen(1,0.8,0.5,1);
				selectable=true;
				criterium=c;
};
bool _sortentry::vergleichen(_sortentry*e){
                if(e==0) return(false);
...
void _previewwindow::zeichnen(_grafik*gra,_kamera<_tg>*cam){
				if(visible){
...
void _previewwindow::animieren(_maus<_tg>*mau,_tastatur*tas){
				animierenkind(mau,tas);
				animierenhilfe(mau);
};
void _previewwindow::setzenstatus(){
};	
//******************************************************************************************************************
//										A C T I V I T Y
//******************************************************************************************************************
...
//******************************************************************************************************************
_statusbarentry::_statusbarentry(_solitaire*s,const _zeichenkette<_tt>&t,_tg o,_tg dt):_listenknotenbasis<_statusbarentry>(&s->statusbarlist){
				solitaire=s;
				text=t;
				opacity=o;
				duration=true;
...
void _statusbarentry::draw(_grafik*gra,_kamera<_tg>*cam){
				...
				gra->rechteck(bild,o.x()-20,o.y(),breite,60,c);
				h=text;
				o.setzen(70+gra->groessex()/2-wm/2,15+12,1);
				if((solitaire->buttonpanel==false)&&(solitaire->menubuttons==false)) o.x(o.x()-340);
				if(o.x()<10) o.x(10);
				c.setzen(1,0.9,0.6,pfadeopacity);
				zeichnenguitext(gra,h,solitaire->font18,o,c);
...
//******************************************************************************************************************
_pileinfoentry::_pileinfoentry(_solitaire*s,const _zeichenkette<_tt>&n,const _zeichenkette<_tt>&d0,const _zeichenkette<_tt>&d1,_to o,_tg dt):_listenknotenbasis<_pileinfoentry>(&s->pileinfolist){
				solitaire=s;
				name=n;
				desc0=d0;
				desc1=d1;
...
void _pileinfoentry::draw(_grafik*gra,_kamera<_tg>*cam){
				...
				zeichnenguitext(gra,h,solitaire->font16,o,c);
				if(desc1!=L""){
				 h=desc1;
...
//										S O L I T A I R E
//******************************************************************************************************************
_solitaire::_solitaire():_guifensterhaupt(L"Grand Solitaire Royale II"){
                initialisiert=false;
				gamestats=0;
				cardsetmanager=0;
				watchdogspeed=0;
				...
				desktopwidth=1024;//wird später geholt
				desktopheight=768;
                //--------------------------------------------------------------------------------------------------------------------------------------------------
...
_solitaire::~_solitaire(){
				L->schreiben("_solitaire : Destruktor : es sind noch speicherbare =",(int)_listebasis<_speicherbarbinaer>::anzahl());
				L->schreiben(" von _game : ",(int)countgamestoreables());
				L->schreiben(" von _pile : ",(int)countpilestoreables());
				L->schreiben(" von _card : ",(int)countcardstoreables());
				L->schreiben(" von _anim : ",(int)countanimationstoreables());
				L->schreiben(" von _stats : ",(int)countstatsstoreables());
				_listebasis<_speicherbarbinaer>::loeschen();
};
HRESULT _solitaire::HandleModeChanges(){
				HRESULT hr;
				hr = dynamic_cast<_grafikdirectx*>(gra)->pdd->TestCooperativeLevel();
				if(SUCCEEDED(hr)||hr==DDERR_WRONGMODE){
			     gra->reinitialisieren();
...
void _solitaire::zeichnen(_guiereignisdaten*){
	//drawstandard();
                           /*     gra->aktivitaet(true);
                                if(gra->szenebeginn()){
                                 gra->validieren();
                                 gra->anzeigebufferloeschfarbe(1,0,0,1);
                                 gra->allebufferloeschen();
                                 gra->ausschussflaeche(false);

                                 gra->transparenz(false);

                                 iggsystem->draw(gra,cam);
                                 gra->szeneende();
                                };
...
void _solitaire::beischliessen(_guiereignisdaten*){
                                zerstoeren();
};
void _solitaire::beigroesse(_guiereignisdaten*){
                                if(gra){
                                 _guirechteck<int> rect=ClientRect<int>();
                                 gra->aktivitaet(true);
                                 gra->groesse(rect.breite(),rect.hoehe());
                                 //animatestandard();
...
void _solitaire::beizerstoeren(_guiereignisdaten*){
                                keyraus=true;
                                PostQuitMessage(0); 
};
void _solitaire::beimauslinksein(_guiereignisdaten*){
                                //akustik->abspielenton(0,tonverwaltung->ton(0),_vektor3<_tg>(0,0,0),_vektor3<_tg>(0,0,0));
};
void _solitaire::beimausbewegung(_guiereignisdaten*e){
                                if(maus){
                                 maus->x(e->maus.x);
                                 maus->y(e->maus.y);
                                };
...
void _solitaire::ladenkurve(_datei*d,_kurvehermite3<_tg>&k){
                _vektor3<_tg> v;
                d->laden(v);
                k.ort(0,v);
                d->laden(v);
...
void _solitaire::speichernkurve(_datei*d,_kurvehermite3<_tg>&k){
                d->speichern(k.ort(0));
                d->speichern(k.ort(1));
                d->speichern(k.vektor(0));
                d->speichern(k.vektor(1));
};

void _solitaire::grafikmodus(unsigned int index){
				...
				*/
				_listebasis<_grafikmodus> grafikmodusliste;
				int style,exstyle;
				if(fullscreen) {
				...
				if(fullscreen){
				 ...
				 _grafikmodus*mode;
				 if(fastvideomode){
				  mode=bestergrafikmodus(&grafikmodusliste,fastvideomodewidth,fastvideomodeheight);
				 }else{
				  mode=bestergrafikmodus(&grafikmodusliste,desktopwidth,desktopheight);
				 };
				 if(mode){
				 ...
				 if(gra==0){
				  gra=new _grafikdirectx(handle(),true,_guifenster::ort()[0],_guifenster::ort()[1],wx,wy,de,ra,0,0);
				  //gra.init(wx,wy,de,ra)
				 }else{
				  if(gra->vollbildmodus()==false){
				   gra->position(_guifenster::ort()[0],_guifenster::ort()[1]);
				  };
				  gra->setzen(true,wx,wy,de,ra);
				 };				
				}else{
				 SetWindowPos(handle(), HWND_TOP, _guifenster::ort()[0],_guifenster::ort()[1],windowedmodewidth,windowedmodeheight, SWP_FRAMECHANGED);
				 if(gra==0){
				  gra=new _grafikdirectx(handle(),false,_guifenster::ort()[0],_guifenster::ort()[1],windowedmodewidth,windowedmodeheight,0,0,0,0);
				 }else{
				  gra->setzen(false,windowedmodewidth,windowedmodeheight,0,0);
				 };
				};
				//Print "Grafikmodus ist : "+gra->groessex()+"*"+gra->groessey()+", "+gra.screen_bpp+"bit, "+gra.screen_freq+"Hz"
				L->schreiben("Grafik modus wurde gesetzt :");
				L->schreiben("Grafik Fenster Ort x = ",gra->positionx());
				L->schreiben("Grafik Fenster Ort y = ",gra->positiony());
				L->schreiben("Grafik Breite = ",(int)gra->breite());
				L->schreiben("Grafik Hoehe = ",(int)gra->hoehe());
				L->schreiben("Grafik Tiefe = ",(int)gra->tiefe());
				L->schreiben("Grafik Rate = ",(int)gra->rate());
...
void _solitaire::loadstatistics(const _zeichenkette<char>&filename){
				...
				if(d.oeffnennurlesen()==false){
				 ...
				 d.laden(typecount);
				 L->schreiben("Wolle a statistik laden : coutn =",(int)typecount);
...
void _solitaire::savestatistics(const _zeichenkette<char>&filename){
				...
				for(unsigned int i=0;i<gametypecount;i++){
				 itoa(si,buf,10);
				 gamestats[i]->speichername()=_zeichenkette<char>("stat")+buf;
				 si++;
				 _gamestatsentry*eit;
				 if(gamestats[i]->entrylist.anfang(eit))do{
				  itoa(ei,buf,10);
				  eit->speichername()=_zeichenkette<char>("statsentry")+buf;
				  ei++;
...
void _solitaire::generatestatistics(){
				L->schreiben("gametypecount=",gametypecount);
				for(unsigned int i=0;i<gametypecount;i++){
				 gamestats[i]=new _gamestats(this,i);
				 for(unsigned int j=0;j<5;j++){
				  _gamestatsentry*eit=new _gamestatsentry(gamestats[i]);
				  if(zufall<unsigned int>(0,1)==0){
				   eit->discards=zufall<unsigned int>(gametype[i].punktemaximal/3,gametype[i].punktemaximal);
				  }else{ 
				   eit->discards=gametype[i].punktemaximal;
				  }; 
				  eit->time=gametype[i].dauer*60*1000+zufall<_tg>(60*500,60*2500);//20*1000+(15*60*1000)*gametypetime[i]+ Rnd(0,5*1000*60)
...
void _solitaire::loadoptions(const _zeichenkette<char>&filename){
				...
				if(d.oeffnennurlesen()==false){ 
				 ...
				 d.laden(farbekerze);
				 d.laden(farbeambienz);
				 if(watchdogspeed) watchdogspeed->load(&d);
				 d.schliessen();
...
void _solitaire::saveoptions(const _zeichenkette<char>&filename){
				...
				if(d.oeffnen()){
				 ...
				 d.speichern(farbekerze);
				 d.speichern(farbeambienz);
				 if(watchdogspeed) watchdogspeed->save(&d);
				 d.schliessen();
				};
};	
_game*_solitaire::newgame(unsigned int cur_game,unsigned int mode, bool prev,bool menue){
				_game*g=0;
				if((cur_game<0)||(cur_game>=gametypecount)) cur_game=3;
				if(cur_game==0){
				 g=new _gameklondikeone(this,mode,prev,menue);
				}else if(cur_game==1){
				 g=new _gameklondikethree(this,mode,prev,menue);
				}else if(cur_game==2){ 
				 g=new _gamespideronesuit(this,mode,prev,menue);
				}else if(cur_game==3 ){
				 g=new _gamespidertwosuit(this,mode,prev,menue);
				}else if(cur_game==4){
				 g=new _gamespiderfoursuit(this,mode,prev,menue);
				}else if(cur_game==5){
				 g=new _gameeightoff(this,mode,prev,menue);
				}else if(cur_game==6){
				 g=new _gamefreecell(this,mode,prev,menue);
				}else if(cur_game==7){
				 g=new _gameforecell(this,mode,prev,menue);
				}else if(cur_game==8){
				 g=new _gameaustralian(this,mode,prev,menue);
				}else if(cur_game==9){
				 g=new _gamebethoral(this,mode,prev,menue);
				}else if(cur_game==10){
				 g=new _gameblindalleys(this,mode,prev,menue);
				}else if(cur_game==11){
				 g=new _gameblanket(this,mode,prev,menue);
				}else if(cur_game==12){
				 g=new _gamecancan(this,mode,prev,menue);
				}else if(cur_game==13){
				 g=new _gamecruel(this,mode,prev,menue);
				}else if(cur_game==14){
				 g=new _gamepyramid(this,mode,prev,menue);
				}else if(cur_game==15){
				 g=new _gamegolf(this,mode,prev,menue);
				}else if(cur_game==16){
				 g=new _gameyukon(this,mode,prev,menue);
				}else if(cur_game==17){
				 g=new _gamefourcolor(this,mode,prev,menue);
				}else if(cur_game==18){
				 g=new _gamescorpion(this,mode,prev,menue);
				}else if(cur_game==19){
				 g=new _gameklondikeonetwodeck(this,mode,prev,menue);
				}else if(cur_game==20){
				 g=new _gameklondikethreetwodeck(this,mode,prev,menue);
				}else if(cur_game==21){
				 g=new _gamerussian(this,mode,prev,menue);
				}else if(cur_game==22){
				 g=new _gameseatowers(this,mode,prev,menue);
				}else if(cur_game==23){
				 g=new _gamelimitedcovered(this,mode,prev,menue);
				}else if(cur_game==24){
				 g=new _gameladypalk(this,mode,prev,menue);
				}else if(cur_game==25){
				 g=new _gamelabyrinth(this,mode,prev,menue);
				}else if(cur_game==26){
				 g=new _gamelabyrintheasier(this,mode,prev,menue);
				}else if(cur_game==27){
				 g=new _gamelabyrinthrodenbach(this,mode,prev,menue);
				}else if(cur_game==28){
				 g=new _gamelabyrinthdouble(this,mode,prev,menue);
				}else if(cur_game==29){
				 g=new _gameosmosis(this,mode,prev,menue);
				}else if(cur_game==30){
				 g=new _gamegolfeasy(this,mode,prev,menue);
				}else if(cur_game==31){
				 g=new _gamekingspeak(this,mode,prev,menue);
				}else if(cur_game==32){
				 g=new _gamefortythieves(this,mode,prev,menue);
				}else if(cur_game==33){
				 g=new _gamelabellelucie(this,mode,prev,menue);
				}else if(cur_game==34){
				 g=new _gamelafacilelucie(this,mode,prev,menue);
				}else if(cur_game==35){
				 g=new _gamerussianeasy(this,mode,prev,menue);
				}else if(cur_game==36){
				 g=new _gamerussianfree(this,mode,prev,menue);
				}else if(cur_game==37){
				 g=new _gameacesup(this,mode,prev,menue);
				}else if(cur_game==38){
				 g=new _gameacme(this,mode,prev,menue);
				}else if(cur_game==39){
				 g=new _gameacesupeasy(this,mode,prev,menue);
				}else if(cur_game==40){
				 g=new _gameant(this,mode,prev,menue);
				}else if(cur_game==41){
				 g=new _gameauntmary(this,mode,prev,menue);
				}else if(cur_game==42){
				 g=new _gameauntfreemary(this,mode,prev,menue);
				}else if(cur_game==43){
				 g=new _gamebakersdozen(this,mode,prev,menue);
				}else if(cur_game==44){
				 g=new _gamebakersgame(this,mode,prev,menue);
				}else if(cur_game==45){
				 g=new _gameflowergarden(this,mode,prev,menue);
				}else if(cur_game==46){
				 g=new _gamegargantua(this,mode,prev,menue);
				}else if(cur_game==47){
				 g=new _gamegiant(this,mode,prev,menue);
				}else if(cur_game==48){
				 g=new _gameindian(this,mode,prev,menue);
				}else if(cur_game==49){
				 g=new _gamemissmilligan(this,mode,prev,menue);
				}else if(cur_game==50){
				 g=new _gameklondikecontra(this,mode,prev,menue);
				}else if(cur_game==51){
				 g=new _gamecontraflow(this,mode,prev,menue);
				}else if(cur_game==52){
				 g=new _gamefreecelldouble(this,mode,prev,menue);
				}else if(cur_game==53){
				 g=new _gamesinglerail(this,mode,prev,menue);
				}else if(cur_game=054){
				 g=new _gamesolidsquare(this,mode,prev,menue);
				}else if(cur_game==55){
				 g=new _gamespiderette(this,mode,prev,menue);
				}else if(cur_game==56){
				 g=new _gamestonewall(this,mode,prev,menue);
				}else if(cur_game==57){
				 g=new _gamespiderlinge(this,mode,prev,menue);
				}else if(cur_game==58){
				 g=new _gamemaze(this,mode,prev,menue);
				}else if(cur_game==59){
				 g=new _gamemartha(this,mode,prev,menue);
				}else if(cur_game==60){
				 g=new _gamelimited(this,mode,prev,menue);
				}else if(cur_game==61){
				 g=new _gamenapoleonssquare(this,mode,prev,menue);
				}else if(cur_game==62){
				 g=new _gamepyramiddouble(this,mode,prev,menue);
				}else if(cur_game==63){
				 g=new _gamefreewall(this,mode,prev,menue);
				}else if(cur_game==64){
				 g=new _gamecarrenapoleon(this,mode,prev,menue);
				}else if(cur_game==65){
				 g=new _gamemazeeasy(this,mode,prev,menue);
				}else if(cur_game==66){
				 g=new _gamemazetwosuit(this,mode,prev,menue);
				}else if(cur_game==67){
				 g=new _gamemazeeasytwosuit(this,mode,prev,menue);
				}else if(cur_game==68){
				 g=new _gameblankettwosuit(this,mode,prev,menue);
				}else if(cur_game==69){
				 g=new _gamepairscross(this,mode,prev,menue);
				}else if(cur_game==70){
				 g=new _gamestonewalleasy(this,mode,prev,menue);
				}else if(cur_game==71){
				 g=new _gamefortythievesdoublestock(this,mode,prev,menue);
				}else if(cur_game==72){
				 g=new _gamefortythievesredeal(this,mode,prev,menue);
				}else if(cur_game==73){
				 g=new _gamepyramidreserve(this,mode,prev,menue);
				}else if(cur_game==74){
				 g=new _gamespidersuperstock(this,mode,prev,menue);
				}else if(cur_game==75){
				 g=new _gamesweetmatch(this,mode,prev,menue);
				}else if(cur_game==76){
				 g=new _gameveryeasy(this,mode,prev,menue);
				};
				return(g);
};
_tg _solitaire::gametypechancemin(unsigned int i){
				_tg c=_tg(gametype[i].gewonnen)/_tg(gametype[i].gewonnen+gametype[i].verloren);
				return(c);
};				
...
void _solitaire::messagestatusbar(const _zeichenkette<_tt>&t,_to o,_tg dt){
				bool exist=false;
				_statusbarentry*sit;
...
void _solitaire::messagepileinfo(const _zeichenkette<_tt>&n,const _zeichenkette<_tt>&d0,const _zeichenkette<_tt>&d1,_to o,_tg dt){
				bool exist=false;
				_pileinfoentry*pit;
...
bool _solitaire::messagebox(const _zeichenkette<_tt>&t,const _zeichenkette<_tt>&tbutton,_tg ox,_tg oy,_zeichensatz*f,_bild<_tb>*iwindow){
				...
				if(lt->anfang(lntit))do{
				 new _iggtext(w0,wx/2-f->breite(*(lntit->objekt()))/2,yit,messageboxtextz,*(lntit->objekt()),tr,tg,tb,f);
				 yit+=25;
...
bool _solitaire::abfragegametype(_zeichensatz*f,_bild<_tb>*iwindow){
			    ...
			    if(currentgame==current_preview){
			     hhh=strgametyperestart1+gametype[currentgame].name+strgametyperestart2;
			     if(abfrage2z(strgametyperestart0,hhh,stryes,strno,gra->groessex()/2,gra->groessey()/2,f,iwindow)==false){
			      return(false);
			     };		
				}else{
			     hhh=strgametypeset+L" "+gametype[currentgame].name+L" "+strto+L" "+gametype[current_preview].name;
...
//-------------------------------------------------------- ANIMATION --------------------------------------------------------------------				
bool _solitaire::animateapplication(){
...
void _solitaire::initgame(unsigned int mode,bool showhighscore){
                deletesecure(game);
				//if(game) delete gamegame.clear(showhighscore);
				game=newgame(currentgame,mode,false,false);
				//game->init(this,mode);
				game->createshadow();
				updatemenumain();
};
void _solitaire::initgamepreview(unsigned int mode,unsigned int number){
				...
				gamepreview=newgame(number,mode,true,false);
...
void _solitaire::initgamemenu(unsigned int mode,unsigned int number){
				...
				gamemenu=newgame(number,mode,false,true);
				//gamemenu->menu=true;
				   ...
				   if(machen){
				    if(cit->shadowarraycount>0){
				     for(unsigned int i=0;i<cit->shadowarraycount;i++){
				      _schatteneinfach<_tg,_to>*sit=cit->shadowarray[i];
				      sit->berechnenopazitaet(cit->opacityquant);
				      sit->mesh->opazitaetverbinder(cit->opacity*sit->opacity*0.35);
				      sit->zeichnen(gra);
				     };
				    };
				   };
...
void _solitaire::drawgamepreview(_tg pox,_tg poy){
				if(gamepreview){
				 ...
				 gamepreview->drawlist.sortieren();
...
void _solitaire::drawgameshadowcurving(bool curving,_to oppile,_to opglobal){
				unsigned int ii=0;
				_pile*pit;
				_card*cit;
				if(game){
				   ...
				   if(machen){
				    for(unsigned int i=0;i<cit->shadowarraycount;i++){//sit:_shadow=EachIn cit.shadowlist
				     ...
				     if(cit->picked==false){
				      ...
				      ii++;
				      sit->mesh->opazitaetverbinder(cit->opacity*sit->opacity*opglobal);
				     }else{
				      sit->opacity=0.7;
				      sit->mesh->opazitaetverbinder(cit->opacity*sit->opacity*opglobal);
			 	     };
...
void _solitaire::transformcameramenu(){
				 ...
				 cammenu->vektor(0,1.75,0);
				 cammenu->vektor().rotx(13);
				 cammenu->vektor().rotz(143);
				 ...
				 world->transformierenglobal(gra,cammenu);
};
void _solitaire::transformcamerapreview(_tg pox,_tg poy){
				 ...
				 world->transformierenglobal(gra,campreview);
};
...
//------------------------------------------------ old effects -------------------------------------------------------------				
_candle*_solitaire::createcandle(_liste<_candle>*l,_tg x,_tg y,_tg z,_tg i,_tg sc,_tg sp,_listebasis<_partikel>*pl){
				_candle*c0;
				bool im=false;
				if(l==&candlemenulist) im=true;
				c0=new _candle(this,x,y,z,i,sc,sp,pl,im);
				if(im){
				 c0->_listenknotenbasis<_candle>::aushaengen();//!!!!!!!!!!!!?????????????? ja ?
				};
				l->einhaengen(c0);
				return(c0);
};
_page*_solitaire::createpage(_textur<_tb>*tf,_textur<_tb>*tb,_textur<_tb>*tex,const _zeichenkette<_tt>&text){
...
void _solitaire::updatelistboxgametypesorted(_igglistbox*tl,unsigned int sel){
				_listesortierung<_sortentry > el;
				_listesortierung<_sortentry> sll;
				...
				if(gametypesortmethod==0){//-------------------alphabetisch
				 for(unsigned int i=0;i<gametypecount;i++){
				  if(gametypeactive[i]){
				   eit=new _sortentry(gametype[i].name,(_tg)gametype[i].name[0]);
				   eit->criterium=0;
				   _tg h=100000000;
				   for(int l=0;l<gametype[i].name.groesse();l++){ 
					eit->criterium+=_tg(gametype[i].name[l])*h;
					h*=0.1;
				   };				   
				   sll.einhaengen(eit);
				  };
				 ...
				 for(unsigned int i=0;i<gametypecount;i++){
				  if(gametypeactive[i]){
				   eit=new _sortentry(gametype[i].name,(1-gametype[i].bekanntheit));
				   sll.einhaengen(eit);
				  };
				 };
				 sll.sortieren();
				 ...
				 for(unsigned int i=0;i<gametypecount;i++){
				  if(gametypeactive[i]){
				   eit=new _sortentry(gametype[i].name,(1-gametypechancemin(i)));
				   sll.einhaengen(eit);
				  };
				 };
				 sll.sortieren();
				 ...
				 for(unsigned int i=0;i<gametypecount;i++){
				  if(gametypeactive[i]){
				   eit=new _sortentry(gametype[i].name,gametype[i].schwierigkeit);
				   sll.einhaengen(eit);
				  };
				 };
				 sll.sortieren();
				 ...
				 for(unsigned int i=0;i<gametypecount;i++){
				  if(gametypeactive[i]){
				   eit=new _sortentry(gametype[i].name,gametype[i].dauer);
				   sll.einhaengen(eit);
				  };
				 };
				 sll.sortieren();
				 ...
				 for(unsigned int i=0;i<gametypecount;i++){
				  if(gametypeactive[i]){
				   eit=new _sortentry(gametype[i].name,gametype[i].gruppe);
				   el.einhaengen(eit);
				  };
				 };
				 int aci=-1;
				 el.sortieren();
				 _listenknoten<_sortentry>*lneit;
				 ...
				 unsigned int j=0;
				 _iggshiftout*gso=0;
				 _iggshiftout*gsoprev=0;
				 
				 if(sll.anfang(lneit))do{//for(Local eit:_sortentry=EachIn  sll
				  if(lneit->objekt()->selectable==false){
				   gsoprev=gso;
				   gso=new _iggshiftout(tl,5,5+i,tl->breite()-50,45,lneit->objekt()->name,-1,1,1,1,font18);
				...
				if(gametypesortmethod!=5){
				 ...
				 //for(Local eit:_sortentry=EachIn  sll
				  ...
				  if(eit->selectable==false){
				   _iggshiftout*gso=new _iggshiftout(tl,5,5+i,tl->breite()-40,40,lneit->objekt()->name,-1,1,1,1,font18);
				   gso->selectable=false;
				   buttoninitmagic(gso->button);
				   i+=40;
				  }else{
				 ...
				 tl->selectbytext(gametype[sel].name);
				 tl->scrolltoselection();
				};
};
void _solitaire::updatemenugametype(){
				...
				for(unsigned int i=0;i<gametypecount;i++){
				 if(typ==gametype[i].name){
				  current_preview=i;
				  current->setzen(gametype[current_preview].name);
				 };
				};
...
void _solitaire::updatemenugametypehelp(_igglistbox*destination,unsigned int gameindex){
				...
				if((gameindex>=0)&&(gameindex<gametypecount)){
				 ...
				 if((gametype[gameindex].gewonnen+gametype[gameindex].verloren)==0) h=L"NOCH NICHT ERMITTELT";
				 _zeichenkette<_tt> hv=int(t*100);
				 //Local sepa$[]=new String[1]
				 //_zeichenkettesepa[0]="."
				 //Local frags$[]=hv.split(sepa[0])
				 hv=L" ("+hv+L"%  "+int(gametype[gameindex].gewonnen)+L" "+strof+L" "+int(gametype[gameindex].gewonnen+gametype[gameindex].verloren)+L")";
				 h+=hv;
				 _iggtext*gtt=destination->inserttext(strchance+L": "+h,5,yit,1,1,0.8,0.5,font18);yit+=off;
				 gtt->createhilfe(strwinchance,font18,0.2,0,0);
				 guihelpinitmagic(gtt);
				 
				 t=gametype[gameindex].schwierigkeit;
				 ...
				 t=gametype[gameindex].dauer;
				 if(t<7) h=strshort;
				 if((t>=7)&&(t<=12)) h=strmedium;
				 if(t>=12) h=strlong;
				 destination->inserttext(strtimeexposure+L" : "+h,5,yit,1,1,0.8,0.5,font18);yit+=off;
   			     destination->inserttext(strdecks+L" : "+gametype[gameindex].deckanzahl,5,yit,1,1,0.8,0.5,font18);yit+=off;
				 ...
				 if(gamepreview){
				  ...
				  for(unsigned int p=0;p<10;p++){
				   ...
				   if(p==0) {if(gamepreview->pilefoundationlist.erstes()) {pl=true;tb=strpilefoundationshort;};};
				   if(p==1) {if(gamepreview->pilediscardlist.erstes()) {pl=true;tb=strpilediscardshort;};};
				   if(p==2) {if(gamepreview->piletableaulist.erstes()) {pl=true;tb=strtableau;};};
				   if(p==3) {if(gamepreview->pilestocklist.erstes()) {pl=true;tb=strpilestock;};};
				   if(p==4) {if(gamepreview->pilewastelist.erstes()) {pl=true;tb=strpilewasteshort;};};
				   if(p==5) {if(gamepreview->pileredeallist.erstes()) {pl=true;tb=strpileredealshort;};};
				   if(p==6) {if(gamepreview->pilereservelist.erstes()) {pl=true;tb=strpilereserveshort;};};
				   if(p==7) {if(gamepreview->pilefreecelllist.erstes()) {pl=true;tb=strpilefreecellshort;};};
				   if(p==8) {if(gamepreview->pilepyramidlist.erstes()) {pl=true;tb=strpilepyramidshort;};};
				   if(p==9) {if(gamepreview->pilelabyrinthlist.erstes()) {pl=true;tb=strpilelabyrinthshort;};};
				   if(pl){
				   // if(pl->anzahl()>0){
				     
				     but=new _iggbutton(destination,xit,yit,160,45,tb,font18,4000+p);
				     buttoninitmagic(but);
					 if(tb==strtableau){//------------------------------------------- tableau pile help description -------------------------------
					  ...
					  //if(gamepreview->tableau){
					   if(gamepreview->piletableaulist.anfang(ptit))do{
					    bool exist=false;
					    if(ptlist.anfang(lnptii))do{//for(Local ptii:_piletableau=EachIn ptlist
					     if(ptit->samerule(lnptii->objekt())) exist=true;
					    }while(ptlist.naechstes(lnptii));
					    if((exist==false)&&(ptit->showdescription)) ptlist.einhaengen(ptit);
					   }while(gamepreview->piletableaulist.naechstes(ptit));
					  //};
					  if(ptlist.anfang(lnptit))do{//for(Local pt:_piletableau=EachIn ptlist
				       ...
				       if(pt->takeruleset()){
				        drophelp=but->createhilfe(hdrop,font18,0.3,125,0,350,true);
						...
						buttontextlist.einhaengen(new _zeichenkette<_tt>(pt->description0+pt->description1));
					   };
					   if(pt->takeemptyruleset()){
					    if(drophelp==0) drophelp=but->createhilfe(hdrop,font18,0.3,125,0,350,true);
						if((pt->takeemptysuit!=-1)||(pt->takeemptynumber!=-1)){
						 dyit=inserthelpsuitnumber(drophelp,dyit,pt,true);
						 if((pt->takeemptysuit!=-1)&&(pt->takeemptynumber==-1)){
						  buttontextlist.einhaengen(new _zeichenkette<_tt>(strdescriptiontakeemptysuit));
						 }else if((pt->takeemptysuit==-1)&&(pt->takeemptynumber!=-1)){
						  if(pt->takeemptynumber==12) buttontextlist.einhaengen(new _zeichenkette<_tt>(strdescriptiontakeemptyking)); else buttontextlist.einhaengen(new _zeichenkette<_tt>(strdescriptiontakeemptynumber));
						 }else if((pt->takeemptysuit!=-1)&&(pt->takeemptynumber!=-1)){
						  if(pt->takeemptynumber==12) buttontextlist.einhaengen(new _zeichenkette<_tt>(strdescriptiontakeemptysuitking)); else buttontextlist.einhaengen(new _zeichenkette<_tt>(strdescriptiontakeemptysuitnumber));
						 };
						};
						if(pt->takeemptymaximal!=-1){
						 ...
						 buttontextlist.einhaengen(new _zeichenkette<_tt>(hh));
						};
						if(pt->takeemptyrestriction){
						 dyit=inserthelprestriction(drophelp,dyit,true);
						 buttontextlist.einhaengen(new _zeichenkette<_tt>(strdescriptiontakeemptyrestriction));
						};
					   }else{
					    if(pt->takerestriction==false) buttontextlist.einhaengen(new _zeichenkette<_tt>(strdescriptiontakeemptyany));
					   };
					   if(drophelp) drophelp->size.y(drophelp->kindhoehe()+10);					   
				       ...
				       if(pt->giveruleset()){
				        ...
				        pickhelp=but->createhilfe(hpick,font18,0.3,125,0,350,true);
						...
						pickhelp->size.y(pickhelp->kindhoehe()+10);
			            ...
			            buttontextlist.einhaengen(new _zeichenkette<_tt>(hpt));
				       }else{
				        buttontextlist.einhaengen(new _zeichenkette<_tt>(strdescriptiongiveanysequence));
					   };
					   ptindex++;
					  ...
					  if(gamepreview->pilefoundationlist.anfang(pfit))do{//for(Local pfit:_pilefoundation=EachIn gamepreview->pilefoundationlist
					   bool exist=false;
					   if(pflist.anfang(lnpfii))do{//for(Local pfii:_pilefoundation=EachIn pflist
					    if(pfit->samerule(lnpfii->objekt())) exist=true;
					   }while(pflist.naechstes(lnpfii));
					   if(exist==false) pflist.einhaengen(pfit);
					  }while(gamepreview->pilefoundationlist.naechstes(pfit));
					  ...
					  if(pflist.anfang(lnpfit))do{//for(Local pf:_pilefoundation=EachIn pflist
				       ...
				       if(pf->takeruleset()){ 
						...
						buttontextlist.einhaengen(new _zeichenkette<_tt>(pf->description0+pf->description1));
						drophelp->size.y(drophelp->kindhoehe()+10);
					   };
				       if(pf->giveruleset()){
				        _zeichenkette<_tt> hpick=strpick;
				        if(pflist.anzahl()>1){
					   ...
					   // buttontextlist.einhaengen(strdescriptiongiverestriction);
					    ...
					    buttontextlist.einhaengen(new _zeichenkette<_tt>(strdescriptiongivefirst));
					   };
 					   pfindex++;
				      ...
				      _pilediscard*pd;
				      if(gamepreview->pilediscardlist.anfang(pd)){
				   	   if(pd->acesupmode){
				        but->createhilfe(strdiscardacesupmode,font18,0.3,100,0,200,true);
					   }else if(pd->pyramidmode){
				        but->createhilfe(strdiscardpyramidmode,font18,0.3,100,0,200,true);
					   }else{	
				        if(pd->showdescription) but->createhilfe(strdiscardnormal,font18,0.3,100,0,200,true);
				  	   };
				       if(but) guihelpinitmagic(but);
 				      };
				     }else if(tb==strpilewasteshort){//--------------------------------------------- waste pile help description ---------------------------------
				      _tg dyit=10;
				      _iggwindow*pickhelp=but->createhilfe(strpick,font18,0.3,125,0,300,true);
					  ...
					  if(gamepreview->pilereservelist.anfang(prit))do{//for(Local prit:_pilereserve=EachIn gamepreview->pilereservelist
					   bool exist=false;
					   if(prlist.anfang(lnprii))do{//for(Local prii:_pilereserve=EachIn prlist
					    if(prit->samerule(lnprii->objekt())) exist=true;
					   }while(prlist.naechstes(lnprii));
					   if(exist==false){
					    prlist.einhaengen(prit);
					    _zeichenkette<_tt> hr=prit->description0+prit->description1;
					    but->createhilfe(hr,font18,0.3,100,0,200,true);
					   };
					  }while(gamepreview->pilereservelist.naechstes(prit));
					  if(but) guihelpinitmagic(but);
				      if(prlist.anfang(lnprii))do{//for(Local pr:_pilereserve=EachIn prlist
					   buttontextlist.einhaengen(new _zeichenkette<_tt>(lnprii->objekt()->description0+lnprii->objekt()->description1));
					  }while(prlist.naechstes(lnprii));
					 }else if(tb==strpileredealshort){//-------------------------------------------- redeal pile help description --------------------------------
					  _pileredeal*peit;
					  if(gamepreview->pileredeallist.anfang(peit))do{//for(Local pit:_pileredeal=EachIn gamepreview->pileredeallist
					   _zeichenkette<_tt> hr=peit->description0+peit->description1;
					   but->createhilfe(hr,font18,0.3,100,0,200,true);
					  }while(gamepreview->pileredeallist.naechstes(peit));
					  if(but) guihelpinitmagic(but);
					  ...
					  if(gamepreview->pilestocklist.anfang(psit))do{//for(Local psit:_pilestock=EachIn gamepreview->pilestocklist
					   bool exist=false;
					   if(pslist.anfang(lnpsii))do{//for(Local psii:_pilestock=EachIn pslist
					    if(psit->samerule(lnpsii->objekt())) exist=true;
					   }while(pslist.naechstes(lnpsii));
					   if(exist==false){
					    pslist.einhaengen(psit);
					    _zeichenkette<_tt> hr=psit->description0+psit->description1;
					    but->createhilfe(hr,font18,0.3,100,0,300,true);
					   };
					  }while(gamepreview->pilestocklist.naechstes(psit));
					  if(but) guihelpinitmagic(but);
				     };//----------------------------------------------------------------------------------------------------------------------------------
					 if(but->hilfe){
					  but->hilfe->visibility(false);
					  but->hilfe->activity(false);
					  but->hilfe->fadeopacity(0)	;
					 };				
				   // };
				   };
				   ...
				   if(but){
				    ...
				    if(ac>0){
				     ...
				     if(buttontextlist.anfang(lnzit))do{//for(Local tab:String=EachIn buttontextlist
				      ...
				      _listenknoten<_zeichenkette<_tt> >*lnzii;
				      if(lt->anfang(lnzii))do{//for(Local tit:String=EachIn lt
				       destination->inserttext(lnzii->objekt(),210,yit+yitl,1,1,0.8,0.5,font18);
				       yitl+=off;
				      }while(lt->naechstes(lnzii));
				      lt->loeschen();
				      deletesecure(lt);
				 ...
				 buttondemoplayplay=new _iggbutton(destination,-200,10,160,40,strdemoplay,font18,1000);
				 buttondemoplayplay->position.addieren(1,0,0,true);
				 buttoninitmagic(buttondemoplayplay);
				 buttondemoplaystop=new _iggbutton(destination,-200,55,160,40,strdemostop,font18,1000);
				 buttondemoplaystop->position.addieren(1,0,0,true);
				 buttoninitmagic(buttondemoplaystop);
				 destination->updatefadeopacity();
				 destination->parent->animieren(maus,tastatur);
				 destination->animieren(maus,tastatur);
				};
};
void _solitaire::inserthelpalteratecolor(_iggwindow*w,_tg y){
				...
				unsigned int suit=zufall<unsigned int>(0,3);
				for(unsigned int i=0;i<7;i++){
				 gg.textur(tsymbolsuit[suit]);
				 if((suit==3)||(suit==0)) suit=zufall<unsigned int>(1,2); else if((suit==1)||(suit==2)) suit=zufall<unsigned int>(0,1)*3;
				 mt=gg.erzeugen();
				 mt->zweiseitig(true);
...
void _solitaire::inserthelpascending(_iggwindow*w,_tg y){
				...
				_tg ch=20;
				_tg x=180;
				unsigned int s=zufall<unsigned int>(0,6);
				_geometrienetz gg(0,_vektor3<_tg>(0,0,0),cw,ch,0);
				unsigned int j=0;
				for(unsigned int i=s;i<=s+6;i++){
				 gg.textur(tsymbolnumbersmall[i]);
				 mt=gg.erzeugen();
				 mt->zweiseitig(true);
				 //mt->ausschussflaeche(false);
				 mt->transparenz(true);
...
void _solitaire::inserthelpboth(_iggwindow*w,_tg y){
				...
				_tg x=180;
				_geometrienetz gg(0,_vektor3<_tg>(0,0,0),cw,ch,0);
				unsigned int n=zufall<unsigned int>(0,12);
				for(unsigned int j=0;j<7;j++){
				 gg.textur(tsymbolnumbersmall[n]);
				 mt=gg.erzeugen();
				 mt->zweiseitig(true);
				 //mt->ausschussflaeche(false);
				 mt->transparenz(true);
...
_tg _solitaire::inserthelpmaximal(_iggwindow*w,_tg y,int m,bool empty){
			 	_tg x=180;
				_iggtext*gt;
...
_tg _solitaire::inserthelprestriction(_iggwindow*w,_tg y,bool empty){
				_zeichenkette<_tt> h;
				if(empty){
			 	_tg x=180;
				 _iggtext*gt;
...
_tg _solitaire::inserthelpsuitnumber(_iggwindow*w,_tg y,_piletableau*pt,bool empty){
				_zeichenkette<_tt> h;
				if(empty){
			 	 _tg x=180;
				 _iggtext*gt;
				...
				if(pt->takeemptysuit!=-1){
				 gg.textur(tsymbolsuit[pt->takeemptysuit]);
				 mt=gg.erzeugen();
				 mt->zweiseitig(true);
				 //mt->ausschussflaeche(false);
				 mt->transparenz(true);
				...
				if(pt->takeemptynumber!=-1){
				 gg.textur(tsymbolnumbersmall[pt->takeemptynumber]);
				 mt=gg.erzeugen();
				 mt->zweiseitig(true);
				 //mt->ausschussflaeche(false);
				 mt->transparenz(true);
...
//******************************************************************************************************************
_camerasolitaire::_camerasolitaire():_kamera<_tg>(0){
	            curvepositiont=0;
	            curveviewt=0;
	            curvesidet=0;
	            curveupt=0;
...
void _camerasolitaire::animate(_zeit*clock){
				...
				if(curving){
				 if(curvepositiont<1){
				  ...
				  curveside.berechnen(curvesidet);
				  curveview.berechnen(curveviewt);
				  curveup.berechnen(curveupt);
				  ...
				  curvesidet=0;
				  curveupt=0;
				  setzen(curveposition.ort(1));
				  vektor(curveview.ort(1));
				  ebene(0,curveside.ort(1));
				  ebene(1,curveup.ort(1));
				  curving=false;
				 };				  
				};				
};
void _camerasolitaire::norm(){
				_vektor3<_tg> h=vektor();
				h%=ebene(0);
				h%=vektor();
				ebene(0,h);
				//view.normalisieren();
